...
export async function withdrawableV2(this: R3vlClient, { walletAddress, isERC20 }: FnArgs) {
  ...
  try {
    if (isERC20) {
      const pendingBalance = await revPathV2.getWithdrawableToken(tokenList[isERC20][_chainId], walletAddress)

      return parseFloat(ethers.utils.formatEther(pendingBalance))
    }

    const pendingBalance = await revPathV2.getWithdrawableToken(ethers.constants.AddressZero, walletAddress)
